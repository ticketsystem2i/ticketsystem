...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($cargo === 'admin') {
        $update = $pdo->prepare("UPDATE tickets SET titulo = ?, descricao = ?, prioridade = ?, area = ? WHERE id = ?");
        $update->execute([$titulo, $descricao, $prioridade, $area, $ticket_id]);
    
    // se o user não tiver cargo, prepara o coamndo para dar update no ticket
    } else {
        $update = $pdo->prepare("UPDATE tickets SET titulo = ?, descricao = ?, prioridade = ?, area = ? WHERE id = ? AND nome_usuario = ?");
        $update->execute([$titulo, $descricao, $prioridade, $area, $ticket_id, $usuario_id]);
    }
